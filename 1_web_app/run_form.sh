...
if [ "$1" == "stop" ]; then
    ...
    PIDS=$(pgrep -f "event_form_app.py" 2>/dev/null)
    if [ ! -z "$PIDS" ]; then
...
if [ "$1" == "start" ] || [ -z "$1" ]; then
    ...
    EXISTING_PID=$(pgrep -f "event_form_app.py" 2>/dev/null)
    ...
    $PIP_CMD install flask > /dev/null 2>&1

    echo "バックグラウンドで登録フォームを起動します..."
    
    # Pythonを直接バックグラウンドで起動（nohupを使わず正確なPIDを取得）
    $PYTHON_CMD system/event_form_app.py > form_app.log 2>&1 &
    ...
    echo "※IPアドレスは変更される場合があります。form_app.log を見ると現在のURLが確認できます。"
    echo ""
